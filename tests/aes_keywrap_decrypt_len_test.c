 ...
 *
 * Regression test for issue F-3852: C_Decrypt with CKM_AES_KEY_WRAP and
 * CKM_AES_KEY_WRAP_PAD must reject ciphertext shorter than two semiblocks
 * (2 * 8 = 16 bytes, RFC 3394) with CKR_ENCRYPTED_DATA_LEN_RANGE. Without that
 * guard the length subtraction (decDataLen = ulEncryptedDataLen - 8) underflows
 * and an undersized ciphertext reaches WP11_AesKeyWrap_Decrypt.
 */
